...
segy_bin_header::segy_bin_header(const vector<byte_t> &raw_data, endian_order swap_endian) {
	initialize(raw_data, swap_endian);
}

void segy_bin_header::initialize(const vector<byte_t> &raw_data, endian_order swap_endian) {
	...
	if (endian_swap == 16909060)
		f_swap_endian = endian_order::none;
	else if (endian_swap == 67305985)
		f_swap_endian = endian_order::reverse;
	else if (endian_swap == 33620995)
		f_swap_endian = endian_order::pair_swap;
	else
		f_swap_endian = swap_endian;

	f_job_id                        = byte_to_int(&raw_data[0], f_swap_endian);
	f_line_num                      = byte_to_int(&raw_data[4], f_swap_endian);
	f_reel_num                      = byte_to_int(&raw_data[8], f_swap_endian);
	f_traces_count                  = (int)byte_to_ushort(&raw_data[12], f_swap_endian);
	f_aux_traces_count              = (int)byte_to_ushort(&raw_data[14], f_swap_endian);
	f_sample_interval               = (double)byte_to_ushort(&raw_data[16], f_swap_endian);
	f_sample_interval_orig          = (double)byte_to_ushort(&raw_data[18], f_swap_endian);
	f_samples_count                 = (int)byte_to_ushort(&raw_data[20], f_swap_endian);
	f_samples_count_orig            = (int)byte_to_ushort(&raw_data[22], f_swap_endian);
	f_data_format                   = (segy_data_format)byte_to_ushort(&raw_data[24], f_swap_endian);

	f_ensemble_fold                 = (int)byte_to_ushort(&raw_data[26], f_swap_endian);
	f_sorting_code                  = (int)byte_to_ushort(&raw_data[28], f_swap_endian);
	f_vert_sum_code                 = (int)byte_to_ushort(&raw_data[30], f_swap_endian);
	f_sweep_fr_start                = (int)byte_to_ushort(&raw_data[32], f_swap_endian);
	f_sweep_fr_end                  = (int)byte_to_ushort(&raw_data[34], f_swap_endian);
	f_sweep_len                     = (int)byte_to_ushort(&raw_data[36], f_swap_endian);
	f_sweep_type                    = (int)byte_to_ushort(&raw_data[38], f_swap_endian);
	f_sweep_chanel_trcs_count       = (int)byte_to_ushort(&raw_data[40], f_swap_endian);
	f_sweep_trc_taper_len_start     = (int)byte_to_ushort(&raw_data[42], f_swap_endian);
	f_sweep_trc_taper_len_end       = (int)byte_to_ushort(&raw_data[44], f_swap_endian);
	f_taper_type                    = (int)byte_to_ushort(&raw_data[46], f_swap_endian);
	f_correlated_traces             = (int)byte_to_ushort(&raw_data[48], f_swap_endian);
	f_gain_recovered                = (int)byte_to_ushort(&raw_data[50], f_swap_endian);
	f_amplitude_rec_method          = (int)byte_to_ushort(&raw_data[52], f_swap_endian);
	f_measurement_system            = (int)byte_to_ushort(&raw_data[54], f_swap_endian);

	f_signal_polarity               = (int)byte_to_ushort(&raw_data[56], f_swap_endian);
	f_polarity_code                 = (int)byte_to_ushort(&raw_data[58], f_swap_endian);

	f_extended_traces_count         = byte_to_int(&raw_data[60], f_swap_endian);
	f_extended_aux_traces_count     = byte_to_int(&raw_data[64], f_swap_endian);
	f_extended_samples_count        = byte_to_int(&raw_data[68], f_swap_endian);
	f_extended_sample_interval      = byte_to_double(&raw_data[72], f_swap_endian);
	f_extended_sample_interval_orig = byte_to_double(&raw_data[80], f_swap_endian);
	f_extended_samples_count_orig   = byte_to_int(&raw_data[88], f_swap_endian);
	f_extended_ensemble_fold        = byte_to_int(&raw_data[92], f_swap_endian);
	...
	f_is_same_for_file              = (int)byte_to_short(&raw_data[302], f_swap_endian);
	f_extended_text_headers_count   = (int)byte_to_short(&raw_data[304], f_swap_endian);
	f_max_add_trc_headers_count     = byte_to_int(&raw_data[306], f_swap_endian);
	f_time_basis                    = (int)byte_to_ushort(&raw_data[310], f_swap_endian);
	f_stream_traces_count           = byte_to_uint64(&raw_data[312], f_swap_endian);
	f_first_trace_offset            = byte_to_uint64(&raw_data[320], f_swap_endian);
...
void segy_bin_header::set_zero() {
	...
	initialize(raw_data, endian_order::none);
}
...
void segy_bin_header::init_map() {
    ...
    { "f_endian_swap",              {(int)f_swap_endian,            seismic_data_type::INT} },
